...
import com.yjfei.pgateway.core.GateCallable;
...
public class AsyncGateServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    	Transaction tran = Cat.getProducer().newTransaction("AsyncGateServlet", "service");
        req.setAttribute("org.apache.catalina.ASYNC_SUPPORTED", true);
        AsyncContext asyncContext = req.startAsync();
        asyncContext.setTimeout(asyncTimeout.get());
        asyncContext.addListener(new AsyncGateListener());
        try {
            poolExecutorRef.get().submit(new GateCallable(asyncContext, gateRunner,tran));
        } catch (RuntimeException e) {
            Cat.logEvent("AsyncGateServlet", "Rejected");
            Cat.logError(e);
